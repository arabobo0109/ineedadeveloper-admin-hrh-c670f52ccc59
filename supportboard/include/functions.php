 ...
 * ==========================================================
 *
 * Main PHP functions file. © 2017-2023 akio.support. All rights reserved.
 *
 */

define('SB_VERSION', '3.6.1');
define("Chat_Log",$_SERVER['DOCUMENT_ROOT']."/log/chat.log");
...
class SBError {
    ...
    function __construct($error_code, $function = '', $message = '') {
        ...
        error_log("\nSBError ".$error_code." ".$function." ".$message, 3,Chat_Log);
    }
        ...
        if ($result) {
            if ($return) {
                if (isset($SB_CONNECTION->insert_id) && $SB_CONNECTION->insert_id > 0) {
                    ...
                    return sb_db_error('sb_db_query with result',$query);
                }
            } else {
                return true;
            }
        } else {
            return sb_db_error('sb_db_query no result',$query);
        }
    } else {
        return $status;
    }
...
function sb_db_error($function,$sql='') {
    global $SB_CONNECTION;
    return new SBError('db-error', $function, $sql." ".$SB_CONNECTION->error);
}
...
function sb_init_translations() {
    ...
    if (!empty($SB_LANGUAGE) && ($SB_LANGUAGE[0] != 'en' || $SB_CLOUD_DEFAULT_LANGUAGE_CODE)) {
        ...
        if ($is_init && !sb_get_setting('front-auto-translations')) {
            $SB_LANGUAGE = [$SB_CLOUD_DEFAULT_LANGUAGE_CODE ? $SB_CLOUD_DEFAULT_LANGUAGE_CODE : 'en', 'front'];
            return;
        }
        $admin = sb_is_agent();
        ...
        if (($language && $language != 'en') && ($admin || isset($_GET['lang']) || $is_init || (sb_get_setting('dialogflow-multilingual-translation') || sb_get_multi_setting('google', 'google-multilingual-translation')))) { // Deprecated: sb_get_setting('dialogflow-multilingual-translation')
            ...
            // if (!((!$language || $language == 'en' || !$is_init) || (!$admin && !isset($_GET['lang']) && !$is_init && (sb_get_setting('dialogflow-multilingual-translation') || sb_get_multi_setting('google', 'google-multilingual-translation'))))) { // Deprecated: sb_get_setting('dialogflow-multilingual-translation')
            switch ($language) {
                case 'nn':
                case 'nb':
                    $language = 'no';
...
function sb_get_versions() {
    return json_decode(sb_download('https://akio.support/synch/versions.json'), true);
}
...
function sb_app_activation($app_name, $key) {
    ...
    $key = trim($key);
    $response = json_decode(sb_download('https://akio.support/synch/updates.php?sb=' . trim($envato_code) . '&' . $app_name . '=' . $key), true);
...
function sb_app_update($app_name, $file_name, $key = false) {
    ...
    $key = trim($key);
    $error = '';
    $zip = sb_download('https://akio.support/synch/temp/' . $file_name);
    if ($zip) {
...
function sb_installation($details, $force = false) {
    ...
    if ($connection_check === true) {
        ...
        // Return
        sb_get('https://akio.support/synch/index.php?site=' . $details['url']);
        ...
        if (is_numeric($interest)) {
            if (!in_array(intval($interest), $agents_ids)) {
                if ($is_user && empty($GLOBALS['SB_FORCE_ADMIN'])) {
                    return new SBError('security-error', 'sb_push_notification');
                }
            } else
                $recipient_agent = true;
        } else {
    ...
    <div class="sb-editor">
        ...
        <?php if ($admin)
            ...
            echo '<div class="sb-btn-attachment" data-sb-tooltip="' . sb_('Attach a file') . '"></div>'; ?>
                ...
                <?php if ($admin && defined('SB_DIALOGFLOW') && sb_get_multi_setting('open-ai', 'open-ai-rewrite'))
                    echo '<div class="sb-btn-open-ai" data-sb-tooltip="' . sb_('Rewrite') . '"></div>'; ?>
                <?php if ($admin && defined('SB_WOOCOMMERCE'))
