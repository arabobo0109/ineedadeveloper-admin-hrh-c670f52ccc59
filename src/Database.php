...
define("DB_Log",$_SERVER['DOCUMENT_ROOT']."/log/db.log");
...
class Database {
     ...
     * @return  void
     */
    public function __construct(PDO $pdo, $table_name="users") {
        $this->conn = $pdo;
    ...
    protected function getResults($query) {
        ...
        $res=$sth->execute(); // Execute statement.
    ...
    public function insert(array $field_value_pairs) {
        ...
        $res=$sth->execute();  //@return bool <b>TRUE</b> on success or <b>FALSE</b> on failure.
        if (!$res){
            self::$error= $sth->errorInfo()[2];
            error_log(self::$error."\n", 3,DB_Log);
            error_log(date("Y-m-d H:i:s")."\n".$query."\n", 3,DB_Log);
            if ($this->tableName=='custom_mail')
                error_log($this->valueParams[0].','.$this->valueParams[1].','.$this->valueParams[3]."\n", 3,DB_Log);
            else
                error_log(print_r($this->valueParams,true)."\n", 3,DB_Log);
        }
        return $res;
    }
     ...
     * @return  \Simcify\Database
     */
    public static function table($table_name='users') {
...
//        $DB=new static($pdo);
    ...
    public function update() {
        ...
        $res=$sth->execute(); // Execute statement.  @return bool <b>TRUE</b> on success or <b>FALSE</b> on failure.
        if (!$res){
            error_log(date("Y-m-d H:i:s")."\n".$query."\n", 3,DB_Log);
            error_log(print_r($this->params,true)."\n", 3,DB_Log);
            self::$error= $sth->errorInfo()[2];
    ...
    {
        return $this->conn->query($this->queryStr)->fetch(PDO::FETCH_ASSOC);
    }

    public function getObjects(): array
    {
        return $this->getResults($this->queryStr);
//        return $this->conn->query($sql)->fetchObject($class_name);
    }

    public function getFirstObj()
    {
        return $this->getResults($this->queryStr)[0];
    }
}
