 ...
 * ==========================================================
 *
 * Email functions file. © 2017-2023 akio.support. All rights reserved.
 *
 * ----------------------------------------------------------
 * EMAIL
 * ----------------------------------------------------------
                        ...
                        if (!in_array($id, $history)) {
                            ...
                            if ($sender && ($follow_up || !sb_is_agent($sender))) {
                                ...
                                if (!empty($message)) {
                                    ...
                                    if (!$follow_up) {
                                        $conversation_id = sb_isset(sb_new_conversation($sender['id'], 2, mb_decode_mimeheader($overview->subject), $department_id, -1, 'em'), 'details', [])['id'];
                                    }
...
function sb_email_get_conversation_code($conversation_id, $count = false, $is_recipient_agent = false) {
    ...
    $messages = sb_db_get('SELECT A.user_id, A.message, A.attachments, A.creation_time, B.first_name, B.last_name, B.profile_image, B.user_type FROM sb_messages A, sb_users B WHERE A.conversation_id = ' . $conversation_id . ' AND A.user_id = B.id ORDER BY A.id ASC', false);
    ...
    for ($i = $start; $i < $count_messages; $i++) {
        ...
        if (!empty($message_text) || count($attachments)) {
            if ($translate && $message_text && $message['user_id'] != $recipient_id) {
...
function sb_remove_email_cron($conversation_id) {
    if (sb_get_setting('notify-email-cron')) {
        if (sb_conversation_security_error($conversation_id)) {
            return new SBError('security-error', 'sb_remove_email_cron');
        }
