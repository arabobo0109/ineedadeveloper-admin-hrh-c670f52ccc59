...
let SignType="signature";  //or "Initials"
...
$(".close-editor-overlay").click(function () {
    if ($('.signer-assembler action').length > 0) {
        notify("Discard Changes?", "Your changes will be lost.", "warning", "Discard Changes", {
    ...
    if (tool === "image") {
        ...
        if (auth) {
            if (isTemplate === "Yes") {
                ...
                notify("Template Only", "Inputs are added to templates only. Do you want to create a template copy of this file?", "warning", "Yes, Create", {
                    showCancelButton: true,
                    closeOnConfirm: true,
...
function assemble(data) {
    ...
    if (data.type === "image" || data.type === Initials || data.type === "signature" || data.type === "symbol" || data.type === "shape" || data.type === "stamp") {
        if (data.group === "field") {
...
$(".signer-builder").on("click", ".signer-element", function () {
    ...
    } else if (att_type === "signature" || att_type === Initials) {
        console.log(att_type+" .signer-builder click");
        if ($(this).attr("group") === "field") {
            if (!auth) {  //sign (guest mode)
                if (sessionStorage.getItem(att_type) === null) {  //show modal to create initials
                    SignType=att_type;
                    $(".text-signature").width(560);
                    $("#sign_dig_title").text("Adopt "+att_type);
                    $("#sign_dlg_label").text("Type your "+att_type);
                    $("#sign_dlg_button").html("Save "+att_type);
                    let textSignature=signature_name;
                    if (att_type===Initials)
                        textSignature=getInitials(textSignature);
                    $(".signature-input").val(textSignature);
                    $(".text-signature span").text(textSignature);
                    $("#updateSignature").modal({show: true, backdrop: 'static', keyboard: false});
                } else {
                    $(this).attr("signed", "true");
                    $(this).find("img").attr("src", sessionStorage.getItem(att_type));
                    console.log(att_type+" .signer-builder click - orgnizeData");
                    orgnizeData(false);
                }
            } else if (signature !== '') {
                $(this).attr("signed", "true");
                ...
                notify("Create signature?", "You don't have a signature yet, create one now on settings page under signature tab.", "info", "Create Signature", {
                    showCancelButton: true,
                    closeOnConfirm: true,
                    callback: "redirect('" + settingsPage + "')"
                });
            }
        }
    }
...
function orgnizeData(prepare) {
    ...
    if (modules.original !== $('#document-viewer').getCanvasImage("image/png")) {
        assemble({
            "type": "drawing",
            "page": pageNum,
            "drawing": $('#document-viewer').getCanvasImage("image/png")
        });
    }
    $('.signer-builder .signer-element').each(function (index, value) {
        ...
        if (actionType === "image" || actionType === Initials || actionType === "signature" || actionType === "symbol" || actionType === "shape" || actionType === "stamp") {
            if (group === "field") {
                if (signerElement.attr("signed") === "false") {
                    emptyAssembler();
                    console.log('resenderPage',pageNumber);
                    renderPage(pageNumber);
                    signerElement.addClass("selected-element");
                    if (prepare)  //when clicked "save" button
                        notify("Sign!", "A signature is required on page " + pageNumber + ". Please sign to continue.", "info", "Sign Now");
                    stopOrganizing = true;
                    return false
                }
            }
            ...
            if (group === "field") {
                ...
                if (!userInput.replace(/\s/g, '').length) {
                    ...
                    renderPage(pageNumber);
                    signerElement.addClass("selected-element");
                    notify("Input!", "An input on page " + pageNumber + " is empty. Please fill to continue.", "info", "Fill Now");
                    stopOrganizing = true;
                    return false
                }
            }
            if (group === "input") {
                if (signerElement.attr("bold") === "true") {
                    bold = "bold";
                }
                if (signerElement.attr("italic") === "true") {
                    italic = "italic";
                }
                if (signerElement.attr("strikethrough") === "true") {
                    strikethrough = "strikethrough";
                }
                if (signerElement.attr("underline") === "true") {
                    underline = "underline";
                }
                if (signerElement.attr("color") !== undefined) {
                    color = signerElement.attr("color");
                }
                if (signerElement.attr("font-size") !== undefined) {
                    fontsize = signerElement.attr("font-size");
                }
                if (signerElement.attr("align") !== undefined) {
                    align = signerElement.attr("align");
                }
                if (signerElement.attr("font") !== undefined) {
                    font = signerElement.attr("font");
                }
                if (signerElement.attr("font") !== undefined) {
                    fontfamily = signerElement.find(".writing-pad").css("font-family");
                }
                ...
                if (signerElement.attr("color") !== undefined) {
                    color = "color: " + signerElement.attr("color") + ";";
                }
                if (signerElement.attr("font-size") !== undefined) {
                    fontsize = signerElement.attr("font-size");
                }
                if (signerElement.attr("align") !== undefined) {
                    align = "text-align: " + signerElement.attr("align") + ";";
                }
                if (signerElement.attr("font") !== undefined) {
                    font = signerElement.attr("font");
                }
    ...
    if (stopOrganizing) {
        ...
        if (prepare) {  //when clicked Save button
            prepareData();  //when clicked Save button
        }
    }
...
$(".input-field-list").on("click", ".input-field-item div", function (event) {
    ...
    if (fieldLabel == "Signature") {
        $('<div class="signer-element selected-element" status="drop" resizeable="true" type="image" group="input" page="' + pageNum + '"><img src="' + baseUrl + 'assets/images/image.png" style="width:200px;opacity:0.5;"></div>').appendTo(".signer-builder");
    }else if (fieldLabel == Initials ){
        $('<div class="signer-element selected-element" status="drop" resizeable="true" type="Initials" group="input" page="' + pageNum + '"><img src="' + baseUrl + 'assets/images/Initials.png" style="width:50px;opacity:0.5;"></div>').appendTo(".signer-builder");
    }else {
...
function showTemplateFields() {
    if (isTemplate === "Yes" && templateFields !== '' && $("body").hasClass("editor") && $(".signer-builder").is(':empty')) {
        ...
        $.each(templateFields, function (i, field) {
            ...
            if (field.type == "image" || field.type == Initials ) {
                $('<div class="signer-element" resizeable="true" type="'+field.type+'" group="input" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + baseUrl + 'assets/images/'+field.type+'.png" style="width:' + signerScale(field.width) + 'px;"></div>').appendTo(".signer-builder");
                initElementsDrag();
                ...
                if (field.strikethrough !== '') {
                    field.strikethrough = ' strikethrough="true"';
                }
                if (field.underline !== '') {
                    field.underline = ' underline="true"';
                }
                if (field.bold !== '') {
                    field.bold = ' bold="true"';
                }
                if (field.italic !== '') {
                    field.italic = ' italic="true"';
                }
...
function acceptRequest() {
    ...
    if ($(".signer-builder").is(':empty')) {
        ...
        if ($("body").hasClass("accept") && requestPositions.length) {
            showLoader();
            if ($(window).width() < 1101) {
                topOffset = 85;//225; //125;//
            } else {
                topOffset = 85;//185;
            }
            console.log('topOffset=',topOffset,'$(window).width()=',$(window).width())
            currentOffset = $(".signer-overlay-previewer").offset();
            currentDocOffset = $("#document-viewer").offset();
            currentPosition = $("#document-viewer").position();
            textInputs = [];
            ...
            $.each(requestPositions, function (i, field) {
                ...
                if (field.type == "image") {
                    $('<div class="signer-element" type="signature" signed="false" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + baseUrl + 'assets/images/image.png" style="width:' + signerScaler(field.width) + 'px;"></div>').appendTo(".signer-builder");
                }
                if (field.type == Initials ) {
                    $('<div class="signer-element" type="Initials" signed="false" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + baseUrl + 'assets/images/Initials.png" style="width:' + signerScaler(field.width) + 'px;"></div>').appendTo(".signer-builder");
                }
                else if (field.type == "text") {
                    // elementId = random({ case: "lower" });
                    elementId = field.text.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-');
                    if (elementId=="Initials"){
                        $('<div class="signer-element" type="initial" signed="false" group="field" page="' + field.page + '" style="display:none;left:' + xPos + 'px;top:' + yPos + 'px;position:absolute;"><img src="' + baseUrl + 'assets/images/Initials.png" style="width:' + signerScaler(field.width) + 'px;"></div>').appendTo(".signer-builder");
                    }
                    else{
...
function updateRequestFields() {
    console.log("updateRequestFields");
...
function loginRequired() {
    notify("Login Required", "You need to login to access this feature.", "warning", "Login Now", {
        showCancelButton: true,
        closeOnConfirm: true,
        callback: "redirect('" + loginPage + "')"
    });
    return false;
}
