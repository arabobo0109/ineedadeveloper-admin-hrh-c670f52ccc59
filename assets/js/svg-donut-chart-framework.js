...
(function ($, doc, win, Snap) {
    ...
    Donut.prototype.drawLegend = function () {
        ...
        var x = 20,//o.center.x + o.radius + 50,
            step = 30,
            radius = 6,
            y = 250;//o.center.y - (data.length / 2 * step) + radius / 2 + (step - radius) / 2;
//amg
    ...
    Donut.prototype.hightlightElement = function (d) {
        ...
        this.animateArcByStroke(d.arc, d.arc.strokeWidth, d.arc.strokeWidth + 10, 500);  //amg
    };
 ...
 * Start framework
 ***********************************************************/
var donutSize = 250; //amg
function drawChart(data, id, chart_datas){
    let chart_data = [
        {
            value: data.created_percent==0?0.01:data.created_percent,
            label: 'Created '+data.created_count+'('+data.created_percent+'%)',
            color: 'purple'
        },
        {
            value: data.occupied_percent==100?99.99:data.occupied_percent,
            label: 'Occupancy '+data.occupied_count+'('+data.occupied_percent+'%)',
            color: '#d9b300'
        },
        {
            value: data.vacant_percent==100?99.99:data.vacant_percent,
            label: 'Vacant '+data.vacant_count+'('+data.vacant_percent+'%)',
            color: '#118dff'
        },
        {
            value: data.unavailable_percent==100?99.99:data.unavailable_percent,
            label: 'Unavailable '+data.unavailable_count+'('+data.unavailable_percent+'%)',
            color: '#5F220E'
        },
        {
            value: data.dirty_percent==100?99.99:data.dirty_percent,
            label: 'Dirty '+data.dirty_count+'('+data.dirty_percent+'%)',
            color: '#240000'
        },
    ];
    if(!id){
        ...
        var donut = $(`#${id}`).donut({
            ...
            radius: donutSize * 0.3 / 1,
            data: chart_datas ? chart_datas : chart_data
        });
