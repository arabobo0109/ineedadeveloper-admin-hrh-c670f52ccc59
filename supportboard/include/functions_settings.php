 ...
 * ==========================================================
 *
 * Settings functions file. © 2017-2023 akio.support. All rights reserved.
 *
 * -----------------------------------------------------------
 * SETTINGS
 * -----------------------------------------------------------
...
function sb_get_front_settings() {
    ...
    $return = [
        ...
        'follow' => sb_get_multi_setting('follow-message', 'follow-active'),
    ...
    if ($return['tickets']) {
        ...
        $return['tickets_first_ticket'] = sb_get_multi_setting('tickets-disable-features', 'tickets-first-ticket');
    }
...
function sb_js_admin() {
    ...
    if (defined('SB_DIALOGFLOW')) {
        ...
        $settings['open_ai_model'] = function_exists('sb_open_ai_get_gpt_model') ? sb_open_ai_get_gpt_model() : 'gpt-3.5-turbo'; // Deprected: function_exists('sb_open_ai_get_gpt_model') ? sb_open_ai_get_gpt_model() : 'gpt-3.5-turbo';
...
function sb_departments($type) {
    ...
    if ($count) {
        switch ($type) {
            case 'select':
                $code = '<div id="department" data-type="select" class="sb-input sb-input-select"><span>' . sb_('Location') . '</span><select><option value=""></option>';
                for ($i = 0; $i < $count; $i++) {
                ...
                break;
            case 'custom-select':
                $code = '<div class="sb-inline sb-inline-departments"><h3>' . sb_('Location') . '</h3><div id="conversation-department" class="sb-select sb-select-colors"><p>' . sb_('None') . '</p><ul><li data-id="" data-value="">' . sb_('None') . '</li>';
                for ($i = 0; $i < $count; $i++) {
...
function sb_system_requirements() {
    ...
    // AJAX file
    $checks['ajax'] = sb_download(SB_URL . '/include/ajax.php');
