 ...
 * ==========================================================
 *
 * Main Javascript admin file. © 2017-2023 akio.support. All rights reserved.
 * 
 */
...
(function ($) {
    ...
    */
            ...
            if (!all && SB_ADMIN_SETTINGS.dialogflow) {
                SBApps.dialogflow.getIntents((response) => {
                    ...
                    for (var i = 0; i < intents.length; i++) {
                        code += `<li><div>${intents[i].displayName}</div><div>${intents[i].messages[0].text.text[0]}</div></li>`;
                    }
                    area.append(code);
                });
            }
    ...
    function whatsapp_direct_message_box(user_ids) {
        ...
        SBF.ajax({
            ...
            if (response.error) return showResponse(response.error.message, 'error');
        ...
        openAI: {
            ...
            rewrite: function (message, greetings = false, onSuccess) {
                SBF.ajax({
                    ...
                    message: `Make the following sentence more friendly and professional and use ${SB_LANGUAGE_CODES[SB_ADMIN_SETTINGS.active_agent_language]} language${greetings ? ', add greetings' : ''}: """${message.replace('"', '\'')}"""`,
                    extra: 'rewrite'
                }, (response) => {
                    if (!response[0]) {
            ...
            crawler: {
                ...
                start: function (urls) {
                    if (urls.length) {
                        ...
                        } else if (this.start_urls.includes(url) && !this.sitemap_processed_urls.includes(url)) {
                            this.active_source = url;
                            ...
                            this.getHTML(url, (response) => {
                                if (response[0].length) {
                                    ...
                                    page.find('a').each(function () {
                                        ...
                                        if (href) {
                                            $(this).html(href.includes('http') ? href : absolute_url(href, SBApps.openAI.crawler.base_url));
                                        }
                                    });
                                        ...
                                        if (text.length > 3500) {
                                            ...
                                            for (var j = 0; j < texts.length; j++) {
                                                if (text.length + texts[j].length < 3500 || j == texts.length - 1) {
                                                    ...
                                                    paragraphs.push([text, response[1]]);
                                                    text = '';
                                                }
                                            }
                                        } else {
                        ...
                        admin.find('#sb-embeddings-box p').html(sb_('We are processing the Support Board articles.'));
                        SBF.ajax({ function: 'open-ai-embeddings-articles' }, () => {
                            ...
                            admin.sbHideLightbox();
                            settings_area.find('#open-ai-train-btn a').sbLoading(false);
                ...
                getHTML: function (url, onSuccess) {
                    ...
                    SBF.ajax({ function: 'get-html', url: url }, function (response) {
                        ...
                        if (response[1] == 200) {
                            ...
                            response = response.substring(response.indexOf('<body'), response.indexOf('</body')).replace('<br>', '\n').replace('<br />', '\n').replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '').replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/src=/g, 'data-src=').replace(/<link /g, '<').replace(/<meta /g, '<');
                ...
                embeddings: function (paragraphs, onSuccess) {
                    SBF.ajax({ function: 'open-ai-get-embeddings', paragraphs: JSON.stringify(paragraphs), save_source: this.active_source }, (response) => {
                        ...
                        if (response[1] == 'chars-limit-exceeded') {
                            dialog(sb_('The chatbot cannot be trained with these sources because the limit of your plan is {R} characters. Upgrade your plan to increase the number of characters.').replace('{R}', response[2]), 'info');
                            onSuccess(false);
                        } else {
                            onSuccess(true);
                        }
    ...
    var SBUsers = {
        ...
        csv: function () {
            SBF.ajax({ function: 'csv-users', users_id: SBUsers.getSelected() }, (response) => {
                dialog(`${sb_('For security reasons, delete the file after downloading it. Close this window to automatically delete it. File location:')}<pre>${response}</pre>`, 'info', false, 'sb-export-users-close', 'Users exported');
                                    ...
                                    if (response_translate) {
                                        for (var i = 0; i < response_translate.length; i++) {
                                            ...
                                            if (message) {
                                                ...
                                                area.find(`[data-id="${message_ids[i]}"] .sb-menu`).prepend(`<li data-value="original">${sb_(SBF.isAgent(message_user_types[i]) && message_user_types[i] != 'bot' ? 'View translation' : 'View original message')}</li>`);
                                            }
                                        }
                                    }
                                    if (SB_ADMIN_SETTINGS.smart_reply) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i].conversation_id)) {
                                ...
                                if (!active_conversation || !conversation) {
                                    if (status == 2 || (SB_ADMIN_SETTINGS.order_by_date && (status == 0 || status == 1))) {
                                ...
                                // Update user
                                if (activeUser() && (payload.event == 'update-user' || (users[user_id].type != item.user_type))) {
                                    activeUser().update(() => {
                                        this.updateUserDetails();
                                ...
                                if (!SBChat.tab_active && item.conversation_status_code == 2 && (!SBF.isAgent(item.message_user_type) || payload.preview) && (!SBF.null(item.message) || !SBF.null(item.attachments) || payload.preview)) {
                                    if (this.desktop_notifications) {
                                        let user_details = [item.first_name + ' ' + item.last_name, SBUsers.userProfileImage(item.profile_image)];
                                        SBChat.desktopNotification(user_details[0], payload.preview ? payload.preview : item.message, user_details[1], conversation_id, user_id);
                                    }
    ...
    */
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            let exclude = ['first_name', 'last_name', 'password', 'profile_image','user_type','department','last_activity'];
            let code = '';
    ...
    */
    ...
    $(document).ready(function () {
        ...
        $(window).keydown(function (e) {
            ...
            if ([13, 27, 32, 37, 38, 39, 40, 46, 90].includes(code)) {
                if (admin.find('.sb-dialog-box').sbActive()) {
                    ...
                    valid = true;
                } else if ([38, 40, 46, 90].includes(code) && conversations_area.sbActive()) {
        ...
        // Updates and apps
        let messages = ['Please go to Settings > Miscellaneous and enter the Envato Purchase Code of Support Board.', `${sb_('Your license key is expired. Please purchase a new license')} <a href="https://akio.support/shop/{R}" target="_blank">${sb_('here')}</a>.`];
        ...
        $(admin).on('click', '.sb-apps > div', function () {
            ...
            box.find('.sb-btn-app-puchase').attr('href', 'https://akio.support/shop/' + app_name + ga);
            box.find('.sb-btn-app-details').attr('href', 'https://akio.support/' + app_name + ga);
            box.sbShowLightbox();
        });
        ...
        */
                    ...
                    for (var i = 0; i < selected_conversations.length; i++) {
                        ...
                        SBF.ajax({
                            ...
                            if (SBApps.is('slack') && [3, 4].includes(status_code)) {
                                SBF.ajax({ function: 'archive-slack-channels', conversation_user_id: SBChat.conversation.user_id });
                            }
                        });
        ...
        $(saved_replies).on('click', '.sb-replies-list li', function () {
            SBChat.insertText($(this).find('div:last-child').html().replace(/\\n/g, '\n'));
            SBF.deactivateAll();
        ...
        $(dialogflow_intent_box).on('change', '#sb-intents-select', function () {
            let intent = $(this).val();
            dialogflow_intent_box.find('.sb-bot-response').css('opacity', intent ? .5 : 1).find('textarea').val(intent ? SBApps.dialogflow.getIntent(intent).messages[0].text.text[0] : '');
        });
        ...
        */
        ...
        $(settings_area).on('click', '#test-email-user .sb-btn, #test-email-agent .sb-btn', function () {
            ...
            if (email && email.indexOf('@') > 0 && !loading(this)) {
                SBF.ajax({
                    ...
                    dialog('Your email has been sent!', 'info');
                    $(this).sbLoading(false);
                });
            }
        });
        ...
        $(settings_area).on('click', '#sb-export-settings a', function (e) {
            ...
            SBF.ajax({
                ...
                window.open(response);
                dialog(`${sb_('For security reasons, delete the file after downloading it. Close this window to automatically delete it. File location:')}<pre>${response}</pre>`, 'info', false, 'sb-export-settings-close', 'Settings exported');
                $(this).sbLoading(false);
            });
            e.preventDefault();
            return false;
        ...
        $(admin).on('click', '#sb-export-settings-close .sb-close, #sb-export-users-close .sb-close', function () {
            SBF.ajax({ function: 'delete-file', path: admin.find('.sb-dialog-box p pre').html() });
        });

        // Slack  
        $(settings_area).on('click', '#slack-button .sb-btn', () => {
            window.open('https://akio.support/synch/?service=slack&plugin_url=' + SB_URL + cloudURL());
            return false;
        });
        ...
        $(settings_area).on('click', '#messenger-button .sb-btn', () => {
            window.open('https://akio.support/synch/?service=messenger&plugin_url=' + SB_URL + cloudURL());
            return false;
        });
