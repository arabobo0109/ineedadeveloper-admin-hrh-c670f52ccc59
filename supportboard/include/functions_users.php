 ...
 * ==========================================================
 *
 * Users functions file. © 2017-2023 akio.support. All rights reserved.
 * 
 * -----------------------------------------------------------
 * LOGIN AND ACCOUNT
 * -----------------------------------------------------------
...
function sb_user_id() {
    return isset($_SESSION['signedUserId'])?$_SESSION['signedUserId'] + 0:0;
}
...
//    $e = new Exception;
//    error_log("\nsb_get_active_user ".print_r($e->getTraceAsString(), true), 3,Chat_Log);
    global $SB_LOGIN;
    if ($SB_LOGIN) {
        return $SB_LOGIN;
    }
    $user_id=sb_user_id();
    if ($user_id==0 && $database!='from_pusher.php'){
        error_log("\nsb_get_active_user no Logined ".print_r($login_data ,true), 3,Chat_Log);
        $e = new Exception;
        error_log("\nsb_get_active_user ".print_r($e->getTraceAsString(), true), 3,Chat_Log);
        sb_logout();
        return false;
    }
    $return=sb_get_user($user_id);
...
function akio_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    $query = 'INSERT INTO sb_users(id, first_name, last_name, email, profile_image, user_type, creation_time, token, department, last_activity) VALUES ("' . sb_db_escape($settings['id']) . '", "' .sb_db_escape($settings['first_name']) . '", "' . sb_db_escape($settings['last_name']) . '", "' . $settings['email'] . '"' . ', "' . sb_db_escape($settings['profile_image']) . '", "' . $settings['user_type'] . '", "' . $settings['creation_time'] . '", "' . $token . '", ' . sb_db_escape($settings['department']) . ', "' . $now . '")';
    sb_db_query($query);
    $user_id =$settings['id'];

    if (!empty($settings_extra)) {
        sb_add_new_user_extra($user_id, $settings_extra);
    }
    if (!sb_is_agent() && ($settings['user_type'] == 'user' || $settings['user_type'] == 'lead' || sb_get_setting('visitor-autodata'))) {
...
function sb_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    if (!empty($settings['id'])) {
        $query = 'INSERT INTO sb_users(id, first_name, last_name, password, email, profile_image, user_type, creation_time, token, department, last_activity) VALUES ("' . sb_db_escape($settings['id']) . '", "' .sb_db_escape($settings['first_name']) . '", "' . sb_db_escape($settings['last_name']) . '", "' . sb_db_escape($password) . '", ' . ($settings['email'] == '' ? 'NULL' : '"' . $settings['email'] . '"') . ', "' . sb_db_escape($settings['profile_image']) . '", "' . $settings['user_type'] . '", "' . $now . '", "' . $token . '", ' . sb_db_escape($settings['department']) . ', "' . $now . '")';
    }
