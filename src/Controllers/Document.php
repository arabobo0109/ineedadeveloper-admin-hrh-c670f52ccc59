...
class Document{
    ...
    public function open($document_key) {
        ...
        $fields = Database::table("fields")->where("type", "custom")->orderBy("id", false)->get();
        $stamps = Database::table("fields")->where("type", "stamp")->orderBy("id", false)->get();
        $inputfields = Database::table("fields")->where("type", "input")->orderBy("id", false)->get();
        ...
        return view('sign', compact("user", "document", "history", "chats", "company","fields","inputfields","templateFields","savedWidth","lauchLabel","request","requestWidth","requestPositions","stamps"));
    }
    ...
    public function fetch() {
        ...
        if (empty($_POST['type']) || $_POST['type'] == "folders") {
            if (!empty(input("search"))) {
                $folders = Database::table("folders")->company()
            ...
            if(input("folder") != 1){
                $subfolders = Database::table("folders")->company()
        ...
        if (empty($_POST['type']) || $_POST['type'] == "files") {
            ...
            if (!empty(input("search"))) {
                $documents = Database::table("files")->company()
    ...
    public function createfolder() {
        header('Content-type: application/json');
        $user = Auth::user();
        $folders = Database::table("folders")->where("name", input("name"))->where("folder", input("folder"))->company()->first();
        if (!empty($folders)) {
    ...
    public function updatefolder() {
        header('Content-type: application/json');
        $folders = Database::table("folders")->where("name", input("foldername"))->where("folder", input("folder"))->company()->first();
        if (!empty($folders)) {
    ...
    public function updatefile() {
        header('Content-type: application/json');
        $user = Auth::user();
        $files = Database::table("files")->where("name", input("filename"))->where("folder", input("folder"))->company()->first();
        if (!empty($files)) {
    ...
    public function duplicate() {
        header('Content-type: application/json');
        $document = Database::table("files")->find(input("file"));
        Signer::duplicate($document);
        exit(json_encode(responder("success", "", "","documentsCallback()", false)));
    }
    ...
    public function updatefolderaccessview() {
        ...
        $folder = Database::table("folders")
                                           ...
                                           ->first();
        $departments = Database::table("departments")->company()->get();
        if ($folder->accessibility == "Departments") {
            $allowedDepartments = json_decode($folder->departments);
        }else{
    ...
    public function updatefileaccess() {
        ...
        $user = Auth::user();
        Database::table("files")->where("id", input("fileid"))->update($data);
        header('Content-type: application/json');
    ...
    public function updatefolderprotectview() {
        $user = Auth::user();
        $folder = Database::table("folders")->company()->where("id", input("folder"))->first();
    ...
    public function updatefolderprotect() {
        header('Content-type: application/json');
        $user = Auth::user();
        $folder = Database::table("folders")->company()->where("id", input("folderid"))->first();
        ...
        Database::table("folders")->company()->where("id", input("folderid"))->update(array("password" => $password));
    ...
    public function send() {
        ...
        foreach ($emails as $email) {
            Mail::send(
                ...
                array(
                    "message" => "Hello there,<br><br>You have received a file from ".$user->fname." ".$user->lname.".<br><strong>Message:</strong> ".input("message")."<br><br>Thank you<br>".env("APP_NAME")." Team"
                ),
                "basic",
                null,
