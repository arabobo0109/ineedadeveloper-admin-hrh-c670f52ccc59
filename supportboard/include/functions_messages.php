 ...
 * ==========================================================
 *
 * Messages functions file. © 2017-2023 akio.support. All rights reserved.
 *
 * -----------------------------------------------------------
 * CONVERSATIONS
 * -----------------------------------------------------------
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $source = false, $extra = false, $tags = false) {
    ...
    $user_id = sb_db_escape($user_id, true);
    if (!sb_isset_num($department)){
        $sender = sb_get_user($user_id);
...
function sb_update_conversation_department($conversation_id, $department, $message = false) {
    if (sb_conversation_security_error($conversation_id)) {
        return new SBError('security-error', 'sb_update_conversation_department');
    }
            ...
            if ((!$queue_active || (!$is_sender_agent && !sb_agents_online())) && !$is_sender_bot && !$is_human_takeover) {
                ...
                if (($agent_check && sb_get_setting('notify-agent-email')) || (!empty($user['email']) && (($user_check && sb_get_setting('notify-user-email')) || $conversation['source'] == 'em'))) {
                    if (sb_get_setting('notify-email-cron')) {
                        ...
                        if (!$continue) {
                            $previous_human_message_user_type = sb_isset(sb_db_get('SELECT A.user_type FROM sb_users A, sb_messages B WHERE B.conversation_id = ' . $conversation_id . ' AND B.id <> ' . $response . ' AND B.user_id = A.id AND (A.user_type = "agent" OR A.user_type = "admin" OR A.id = ' . $user_id . ') ORDER BY B.id DESC LIMIT 1'), 'user_type');
                            $continue = !$previous_human_message_user_type || sb_is_agent($previous_human_message_user_type);
                        }
                        if ($continue) {
...
function sb_tags_update($conversation_id, $tags, $add = false) {
    if (sb_conversation_security_error($conversation_id)) {
        return new SBError('security-error', 'sb_tags_update');
    }
...
function sb_execute_bot_message($name, $conversation_id, $last_user_message = false, $check = true) {
    ...
    if (!$is_check && sb_conversation_security_error($conversation_id)) {
        return new SBError('security-error', 'sb_execute_bot_message');
    }
    switch ($name) {
        case 'offline':
